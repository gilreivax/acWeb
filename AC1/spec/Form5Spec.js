describe('Form 5', () => {
  ...
  describe('Observações', () => {
    ...
    it('deve possuir um texto de sugestão', () => expect(this.input.placeholder).withContext('Existe uma propriedade que coloca um texto que é exibido quando o campo está vazio').toBe('Por favor, escreva aqui suas observações!'))
    it('deve possuir um label correto', () => expect(document.querySelector('#form5 label[for="' + this.input.id + '"]')).withContext('Labels devem estar corretamente conectados aos respectivos campos').toBeTruthy())
  })
